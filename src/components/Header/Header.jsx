...
const menuArray = [
  ...
  },
  {
    title: "세대안내",
    subMenu: [
      { subTitle: "64A", subUrl: "/FloorPlan/59A" },
      { subTitle: "64B", subUrl: "/FloorPlan/59B" },
      ...
      <>
        ...
        <div className={styles.mobileHeader}>
          <div onClick={() => setIsMobileMenu((v) => !v)}>
            {!isMobileMenu ? (
              <AiOutlineMenu className={styles.icon} size={25} color="#17453a" />
            ) : (
              <IoCloseSharp className={styles.icon} size={25} color="#17453a" />
            )}
          </div>
          {isMobileMenu && (
            <SlideMenu
          ...
          <a href="tel:1533-8848">
            <IoCall className={styles.icon} size={25} color="#17453a" />
          </a>
        </div>
        {isInterestPopupOpen && (
          <InterestPopup
